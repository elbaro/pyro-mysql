...
pub fn bench_sqlalchemy_async(c: &mut Criterion) {
    ...
    Python::attach(|py| {
        Python::run(
            py,
            c_str!(include_str!("./sqlalchemy_async.py")),
            None,
            None,
        )
        .unwrap();
    ...
    for select_size in [1, 10, 100, 1000] {
        ...
        for name in ["pyro/zero (async)", "aiomysql (async)", "asyncmy (async)"] {
            group.bench_function(name, |b| {
                Python::attach(|py| {
                    Python::run(
                        py,
                        &std::ffi::CString::new(format!(
                            "session, engine = create_async_session('{name}')"
                        ))
                        .unwrap(),
                        None,
                        None,
                    ...
                    b.iter(|| {
                        py.eval(c"loop.run_until_complete(select_query(session))", None, None)
                            .unwrap()
                    });

                    Python::run(
                        py,
                        c"loop.run_until_complete(session.close()); loop.run_until_complete(engine.dispose())",
                        None,
                        None,
                    )
                    .unwrap();
    ...
    // Benchmark INSERT operations (individual)
    {
        ...
        for (name, stmt_template) in [
            ("pyro/zero (async)", "insert_individual(session, {})"),
            ("aiomysql (async)", "insert_individual(session, {})"),
            ("asyncmy (async)", "insert_individual(session, {})"),
        ] {
            group.bench_function(name, |b| {
                Python::attach(|py| {
                    Python::run(
                    ...
                    b.iter_custom(|iters| {
                        ...
                        for g in 0..((iters - 1) / 10000 + 1) {
                            ...
                            let statement = format!(
                                ...
                                stmt_template.replace("{}", &(end - start).to_string())
                            );
